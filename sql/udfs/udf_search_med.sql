...
BEGIN
		...
		FROM db.Medicamento JOIN db.Lotes ON db.Medicamento.nome=db.Lotes.nome_med AND db.Medicamento.lab_id=db.Lotes.lab_id WHERE Lotes.quantidade > 0 AND Medicamento.codigo=@codigo;
RETURN;
END;
