CREATE FUNCTION db.udf_historico_meds_without_pres(@nif int = null)
...
BEGIN
	...
	ELSE
		BEGIN
            INSERT @table SELECT Venda.utente_NIF, Venda.func_NIF, Venda.data, Venda.num_venda, temmv.nome, temmv.lab_NIPC
            FROM db.venda LEFT OUTER JOIN db.prescricao ON db.venda.num_venda=db.prescricao.num_venda JOIN db.temmv ON db.venda.num_venda=db.temmv.num_venda WHERE db.prescricao.num_prescricao is null AND venda.utente_NIF=@nif;
		END;
RETURN;
END;