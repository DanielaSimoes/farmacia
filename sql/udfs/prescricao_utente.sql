...
AS
BEGIN
    DECLARE db_cursor CURSOR FOR SELECT  Contem.nome_medicamento as nome, Contem.lab_NIPC, COUNT(*) as num_unidades FROM db.Contem WHERE db.Contem.num_prescricao=@num_prescricao GROUP BY Contem.nome_medicamento, Contem.lab_NIPC
    ...
    BEGIN
            ...
            SELECT @count = count(*) FROM (db.TemMV JOIN db.Venda ON db.TemMV.num_venda=db.Venda.num_venda)
                    ...
                    WHERE db.Prescricao.num_prescricao=@num_prescricao AND TemMV.nome=@nome, TemMV.lab_NIPC=@lab_NIPC;
